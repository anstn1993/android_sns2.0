...
import com.example.sns.chat.activity.ChatActivity;
...
import com.example.sns.post.edit.activity.EditVideoPostFirstActivity;
...
public class PostActivity extends AppCompatActivity implements PostAdapter.PostRecyclerViewListener, HttpRequest.OnHttpResponseListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_chat = findViewById(R.id.btn_chat);//채팅 버튼
        ...
        //addOnScrollListener는 리사이클러뷰를 스크롤 리스너
        rvPost.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                ...
                //현재 화면에서 가장 처음으로 보이는 아이템의 index
                int firstVisibleItemPosition = ((LinearLayoutManager) recyclerView.getLayoutManager()).findFirstVisibleItemPosition();
                //현재 화면에서 가장 마지막으로 보이는 아이템의 index
                int lastVisibleItemPosition = ((LinearLayoutManager) recyclerView.getLayoutManager()).findLastVisibleItemPosition();
                //리사이클러뷰의 아이템 개수를 담는다.
                int totalCount = recyclerView.getAdapter().getItemCount();
                //현재 화면에 보이는 아이템의 개수를 담는다.(1~2)
                ...
                //현재 화면에 보이는 아이템의 개수가 1인 경우는 제외해야 스크롤을 할때마다 화면에 보이는 마지막 아이템의 인덱스와
                //마지막 아이템의 인덱스가 계속 같아서 다음 페이지를 가져오는 로직을 실행하지 않는다.
    ...
    public void setRecyclerView() {
        ...
        rvPost = findViewById(R.id.recyclerview_post);
    ...
    //서버로부터 미확인 메세지(채팅)의 수를 json형태로 받아오는 메소드
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    requestBody.put("lastPostNum", postItemArrayList.get(postItemArrayList.size() - 2).getPostNum());//가장 마지막 게시물 id
    ...
    private void processLike(boolean isLike, int postNum, int position) {
        try {
            JSONObject requestBody = new JSONObject();
            //좋아요 등록, 취소에 따라 서버에서의 처리 로직이 달라진다.
            if (isLike) {
    ...
    protected void onStart() {
        ...
        if (isUploaded) {
            Log.d("게시물 새로고침", "호출");
            rvPost.smoothScrollToPosition(0);//게시물이 업로드되었기 때문에 리사이클러뷰의 최상단으로 이동시킨다.
            isUploaded = false;
        }
        ...
        if (postItemArrayList.get(position).getIsMyPost()) {
            ...
            tv_editpost.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (postItemArrayList.get(position).getType().equals("image")) {
                    ...
                    //수정하려는 게시물이 동영상 게시물인 경우
                        ...
                        intent.putExtra("uri", "http://13.124.105.47/uploadvideo/" + postItemArrayList.get(position).getVideo());//게시물의 동영상 파일명
                    }
                    //게시물 번호를 넘겨준다.
                ...
                public void onClick(View v) {
                    ...
                    tv_yes.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            deletePost(postItemArrayList.get(position).getPostNum(), position, postItemArrayList.get(position).getType());//게시물 삭제 메소드
                        }
                    });
        ...
        try {
            ...
            if (requestType.equals("messageCount")) {//메세지 개수를 받아오는 통신을 한 경우
                ...
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        int firstVisibleItemPosition = ((LinearLayoutManager) rvPost.getLayoutManager()).findFirstVisibleItemPosition();
                        int lastVisibleItemPosition = ((LinearLayoutManager) rvPost.getLayoutManager()).findLastVisibleItemPosition();
                        //가장 첫번째로 보이는 뷰
                        View firstItemView = rvPost.getLayoutManager().findViewByPosition(firstVisibleItemPosition);
                        //가장 마지막으로 보이는 뷰
                        View lastItemView = rvPost.getLayoutManager().findViewByPosition(lastVisibleItemPosition);
                        //첫번째 아이템이 화면에 보이는 비중
                        float firstItemPercentage = ((firstItemView.getHeight() - Math.abs(firstItemView.getY())) / firstItemView.getHeight()) * 100;
                        //두번째 아이템이 화면에 보이는 비중
                        float lastItemPercentage = ((lastItemView.getHeight() - Math.abs(lastItemView.getY())) / lastItemView.getHeight()) * 100;
