...
public class SearchedTagAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    interface SearchedTagRecyclerViewListener{
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int i) {
        ...
        if(holder instanceof SearchedTagViewHolder){
            ...
            //대표 사진 설정
            if(searchedTagItemArrayList.get(i).type.equals("image")) {//이미지 게시물인 경우
                Glide.with(context).load("http://13.124.105.47/uploadimage/"+searchedTagItemArrayList.get(i).image)
                        .thumbnail(0.1f)
                        .apply(new RequestOptions().centerCrop())
                        .into(((SearchedTagViewHolder)holder).cv_image);
            }
            else {//비디오 게시물인 경우
                Glide.with(context).load("http://13.124.105.47/uploadvideo/"+searchedTagItemArrayList.get(i).video)
                        .thumbnail(0.1f)
            ...
            //태그 설정
            ((SearchedTagViewHolder)holder).tv_tag.setText("#"+searchedTagItemArrayList.get(i).tag);
            //게시물 수 설정
            ((SearchedTagViewHolder)holder).tv_count.setText("게시물 "+searchedTagItemArrayList.get(i).count+"개");
