...
public class FollowAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    interface FollowListRecyclerViewListener {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        //프로필 사진 설정
        Glide.with(context).load("http://13.124.105.47/profileimage/"+followListItemArrayList.get(position).profile)
                .thumbnail(0.1f)
        ...
        //팔로우를 하고 있는 상태인 경우
        else if(followListItemArrayList.get(position).isFollowing){
            //팔로잉 버튼으로 설정
