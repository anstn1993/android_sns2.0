package com.example.sns.like.adapter;
...
public class LikeListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public interface LikeListRecyclerViewListener{
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        //프로필 사진 설정
        Glide.with(context).load("http://13.124.105.47/profileimage/"+likeListItemArrayList.get(position).getProfile())
                .thumbnail(0.1f)
        ...
        //닉네임 설정
        ((LikeListItemHolder)holder).tv_nickname.setText(likeListItemArrayList.get(position).getNickname());


        //현재 로그인 아이디와 좋아요한 계정이 일치하는 경우
        if(likeListItemArrayList.get(position).getAccount().equals(LoginUser.getInstance().getAccount())){
            //팔로우 버튼을 없앤다.
            ((LikeListItemHolder)holder).btn_follow.setVisibility(View.GONE);
        }
        //팔로우를 하고 있는 상태인 경우
        else if(likeListItemArrayList.get(position).getIsFollowing()){
            //팔로잉 버튼으로 설정
