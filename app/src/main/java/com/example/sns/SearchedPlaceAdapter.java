...
public class SearchedPlaceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    //아이템 어레이리스트
    ArrayList<SearchedPlaceItem> searchedPlaceItemArrayList = new ArrayList<>();
    Context context;
    ...
    interface SearchedPlaceRecyclerViewListener{
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof SearchedPlaceViewHolder){
            //장소 설정
            ((SearchedPlaceViewHolder) holder).tv_place.setText(searchedPlaceItemArrayList.get(position).address);
            //개수 설정
            ((SearchedPlaceViewHolder) holder).tv_count.setText("게시물 "+searchedPlaceItemArrayList.get(position).totalCount+"개");
            //아이템 클릭 설정
