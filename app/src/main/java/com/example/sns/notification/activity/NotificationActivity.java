package com.example.sns.notification.activity;
...
import com.example.sns.notification.adapter.NotificationAdapter;
import com.example.sns.notification.model.NotificationItem;
import com.example.sns.notification.receiver.NotificationReceiver;
...
import com.example.sns.follow.fragment.FollowerListFragment;
import com.example.sns.follow.fragment.FollowingListFragment;
...
import com.example.sns.mypage.fragment.AccountPageFragment;
...
import static com.example.sns.main.activity.MainActivity.iv_notificationDot;
...
public class NotificationActivity extends AppCompatActivity implements NotificationAdapter.NotificationRecyclerViewListener, HttpRequest.OnHttpResponseListener, NotificationReceiver.OnPushNotificationListener {
    ...
    public void getNotificationData(String requestType, String account, int lastId, int listSize) {
        try {
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    requestBody.put("lastId", notificationItemArrayList.get(notificationItemArrayList.size() - 2).getId());
                ...
                if (requestType.equals("getNotification")) {//알림 데이터들을 가져오는 통신
                    ...
                    if (isFromPush) {
                        ...
                        if (notificationItemArrayList.size() != 0) {
                            ...
                            updateCheckNotification(notificationItemArrayList.get(0).getId());
                        }

                        //좋아요 알림인 경우
                        if (category.equals("like")) {
                            //아직 알림을 확인하지 않은 경우
                            if (notificationItemArrayList.get(0).getIsChecked() == false) {
                                //해당 알림을 확인했기 때문에 알림리스트의 배경 색이 흰색으로 바뀌게 한다.
                                notificationItemArrayList.get(0).setIsChecked(true);
                        ...
                        //팔로우 알림인 경우
                            ...
                            if (notificationItemArrayList.get(0).getIsChecked() == false) {//아직 알림을 확인하지 않은 경우
                                notificationItemArrayList.get(0).setIsChecked(true);//해당 알림을 확인했기 때문에 알림리스트의 배경 색이 흰색으로 바뀌게 한다.
                                notificationAdapter.notifyItemChanged(0, "checked");
                            }
                    ...
                    if (isFollowing) {
                        //팔로우 상태를 true로 만든다.
                        String userAccount = notificationItemArrayList.get(position).getUserAccount();
                        for (int i = 0; i < notificationItemArrayList.size() - 1; i++) {
                            if (notificationItemArrayList.get(i).getUserAccount().equals(userAccount)) {
                                notificationItemArrayList.get(i).setIsFollowing(true);
                                //어댑터에 notify
                        ...
                        String receiver = notificationItemArrayList.get(position).getUserAccount();
                        String title = "SNS";
                    ...
                    //팔로우를 취소한 경우
                        ...
                        for (int i = 0; i < notificationItemArrayList.size() - 1; i++) {
                            if (notificationItemArrayList.get(i).getUserAccount().equals(userAccount)) {
                                notificationItemArrayList.get(i).setIsFollowing(false);
                                //어댑터에 notify
    ...
    public void onProfileClicked(int position) {
        ...
        //게시물 번호를 번들에 담아서 프래그먼트에 전달해준다.
        Bundle bundle = new Bundle();
        bundle.putString("account", notificationItemArrayList.get(position).getUserAccount());
        bundle.putString("parentActivity", "NotificationActivity");
        bundle.putBoolean("isMyPost", false);
        NotificationActivity.accountPageFragment.setArguments(bundle);
        NotificationActivity.fragmentManager.beginTransaction().setCustomAnimations(R.anim.enter_from_right, R.anim.exit_to_right, R.anim.enter_from_right, R.anim.exit_to_right).add(R.id.frame_parent_container, NotificationActivity.accountPageFragment).addToBackStack(null).commit();
    ...
    public void onFollowClicked(int position) {
        ...
        String followedAccount = notificationItemArrayList.get(position).getUserAccount();

        //해당 position의 사용자 nickname
        String followedNickname = notificationItemArrayList.get(position).getNickname();
        ...
        //팔로우를 했던 상태에서 팔로우를 취소하려는 경우
            ...
            //프로필 이미지
            String image = notificationItemArrayList.get(position).getProfile();
