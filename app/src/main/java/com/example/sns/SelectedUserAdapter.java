...
public class SelectedUserAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        //선택된 사용자 프로필 이미지 설정
        Glide.with(context).load("http://13.124.105.47/profileimage/"+selectedUserItemArrayList.get(position).profile)
                .thumbnail(0.1f)
        ...
        //선택된 사용자 닉네임 설정
        ((SelectedUserViewHolder) holder).tv_nickname.setText(selectedUserItemArrayList.get(position).nickname);
