...
public class GridPostAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    //뷰타입 설정
    public final int VIEW_POST = 0;
    public final int VIEW_PROGRESS = 1;
    ...
    public interface GridPostRecyclerViewListener {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int i) {
        if (postItemArrayList.get(i).getType().equals("image")) {//이미지 게시물인 경우
            //게시물 이미지 설정(첫번째 이미지로 설정한다)
            Glide.with(context).load("http://13.124.105.47/uploadimage/" + postItemArrayList.get(i).getImageList().get(0))
                    .thumbnail(0.1f)
            ...
            //게시물의 이미지 수가 1개인 경우
            if (postItemArrayList.get(i).getImageList().size() == 1) {
                //다중 이미지 아이콘 없앰
    ...
    private class GridViewHolder extends RecyclerView.ViewHolder {
        ...
        private ImageView iv_image, iv_multiple, iv_video;
