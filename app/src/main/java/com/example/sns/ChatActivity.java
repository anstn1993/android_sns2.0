...
public class ChatActivity extends AppCompatActivity implements ChatRoomAdapter.ChatRoomRecyclerViewListener, HttpRequest.OnHttpResponseListener {
    ...
    private void updateChatRoom(int position, String message, String type, String time) {
        //읽지 않은 메세지 수 +1
        chatRoomItemArrayList.get(position).setNewMessageCount(chatRoomItemArrayList.get(position).newMessageCount + 1);
        //새로운 메세지 수신 시간 설정
    ...
    private void updateChatRoom(int position, String message, String type, String time, String account) {
        //나간 사용자의 데이터를 삭제.
        for (int i = 0; i < chatRoomItemArrayList.get(position).userList.size(); i++) {
            if (account.equals(chatRoomItemArrayList.get(position).userList.get(i).account)) {
                chatRoomItemArrayList.get(position).userList.remove(i);
            }
        }
    ...
    private void updateChatRoom(int position, String message, String type, String time, JSONArray addedParticipantList) {
        ...
        for (int i = 0; i < addedParticipantList.length(); i++) {
            try {
                ...
                chatRoomItemArrayList.get(position).userList.add(new ChatRoomItem.User(account, nickname, profile));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private void exitChatRoom(int position) {
        ...
        try {
            ...
            for (int i = 0; i < chatRoomItemArrayList.get(position).userList.size(); i++) {
                //수신자 닉네임
                String receiverAccount = chatRoomItemArrayList.get(position).userList.get(i).account;
                ...
                if (requestType.equals("getChatRoom")) {//채팅방 목록을 가져오는 통신
                    ...
                    if (isFromChatRoom) {
                        ...
                        for (int i = 0; i < chatRoomItemArrayList.size(); i++) {
                            ...
                            if (chatRoomItemArrayList.get(i).userList.size() == 1) {
                                //기존에 존재하는 1:1채팅방의 상대방 계정
                                String account = chatRoomItemArrayList.get(i).userList.get(0).account;
                                ...
                                if (selectedUserAccount.equals(account)) {
                                    ...
                                    try {
                                        ...
                                        jsonObject.put("nickname", chatRoomItemArrayList.get(i).userList.get(0).nickname);
                                        jsonObject.put("profile", chatRoomItemArrayList.get(i).userList.get(0).profile);
                                    } catch (JSONException e) {
                                        e.printStackTrace();
                                    }
                                    ...
                                    for (int j = 0; j < chatRoomItemArrayList.size(); j++) {
                                        if (chatRoomItemArrayList.get(j).newMessageCount != 0) {
                                            newMessageCount += chatRoomItemArrayList.get(j).newMessageCount;
                                        }
                                    }
                                    //미확인 메세지가 있는 방을 들어가는 경우에는 그 방의 미확인 메세지 수만큼 빼서 넘겨준다.
                                    if (chatRoomItemArrayList.get(i).newMessageCount != 0) {
                                        newMessageCount -= chatRoomItemArrayList.get(i).newMessageCount;
                    ...
                    createdRoomData.put("roomNum", chatRoomItem.roomNum);
                    //채팅방 참여자의 계정, 닉네임, 프로필 데이터를 담는 json 객체를 담을 json배열
                    JSONArray participantList = new JSONArray();

                    for (int j = 0; j < chatRoomItem.userList.size(); j++) {
                        //참여자의 계정, 닉네임, 프로필 데이터를 담는 json
                        JSONObject userDataJSON = new JSONObject();
                        userDataJSON.put("account", chatRoomItem.userList.get(j).account);
                        userDataJSON.put("nickname", chatRoomItem.userList.get(j).nickname);
                        userDataJSON.put("profile", chatRoomItem.userList.get(j).profile);
    ...
    public void onContainerClicked(int position) {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                JSONObject jsonObject = new JSONObject();
                String account = chatRoomItemArrayList.get(position).userList.get(i).account;
                String nickname = chatRoomItemArrayList.get(position).userList.get(i).nickname;
                String profile = chatRoomItemArrayList.get(position).userList.get(i).profile;
                try {
                    jsonObject.put("account", account);
        ...
        if (chatRoomItemArrayList.get(position).newMessageCount != 0) {
            newMessageCount -= chatRoomItemArrayList.get(position).newMessageCount;
            intent.putExtra("newMessageCount", newMessageCount);
        }
        //그렇지 않은 경우에는 그냥 넘겨준다.
        else {
