...
public class ChatRoomAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    //시간 포맷
    private final String fromFromat = "yyyy-MM-dd HH:mm:ss";
    private final String toFormat = "M월 d일";
    ...
    //리사이클러뷰 클릭 인터페이스
    public interface ChatRoomRecyclerViewListener {
        void onContainerClicked(int position);

        void onExitRoomClicked(int position);
    }

    private ChatRoomRecyclerViewListener mListener;
    ...
    public int getItemViewType(int position) {
        //채팅방에 자기밖에 안 남은 경우
        if (chatRoomItemArrayList.get(position).getUserList().size() == 0) {
            return VIEW_NONE;
        }
        //채팅방의 참여자 수가 2명인 경우(자기 포함)
        else if (chatRoomItemArrayList.get(position).getUserList().size() == 1) {
            return VIEW_ONE;
        }
        //채팅방의 참여자 수가 3명인 경우(자기 포함)
        else if (chatRoomItemArrayList.get(position).getUserList().size() == 2) {
            return VIEW_TWO;
        }
        //채팅방의 참여자 수가 4명인 경우(자기 포함)
        else if (chatRoomItemArrayList.get(position).getUserList().size() == 3) {
            return VIEW_THREE;
        }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof ChatListOneViewHolder) {
            ...
            String nickname = chatRoomItemArrayList.get(position).getUserList().get(0).getNickname();
            String profile = chatRoomItemArrayList.get(position).getUserList().get(0).getProfile();
            //채팅방 상대의 프로필 사진 설정
            ...
            if (chatRoomItemArrayList.get(position).getMessage() != null) {
                //채팅방 마지막 메세지 설정
                String message = chatRoomItemArrayList.get(position).getMessage();
                ((ChatListOneViewHolder) holder).tv_content.setText(message);
            }

            if (chatRoomItemArrayList.get(position).getTime() != null) {
                //채팅방 마지막 메세지 도착 시간 설정
                String time = ChatContentDetailActivity.formatDate(chatRoomItemArrayList.get(position).getTime(), fromFromat, toFormat);
            ...
            //미확인 메세지가 존재하는 경우
                ...
                int newMessageCount = chatRoomItemArrayList.get(position).getNewMessageCount();
                ((ChatListOneViewHolder) holder).tv_messageCount.setText(String.valueOf(newMessageCount));
            }

            viewBinderHelper.bind(((ChatListOneViewHolder) holder).swipeLayout, String.valueOf(chatRoomItemArrayList.get(position).getRoomNum()));
        ...
        //두명과 대화하는 채팅방 리스트
            ...
            String nickname2 = chatRoomItemArrayList.get(position).getUserList().get(1).getNickname();
            String profile2 = chatRoomItemArrayList.get(position).getUserList().get(1).getProfile();
            //채팅방 상대의 프로필 사진 설정
            ...
            viewBinderHelper.bind(((ChatListTwoViewHolder) holder).swipeLayout, String.valueOf(chatRoomItemArrayList.get(position).getRoomNum()));
            //스와이프하면 나오는 나가기 버튼 클릭 리스너
        ...
        //세명과 대화하는 채팅방 리스트
            ...
            String nickname3 = chatRoomItemArrayList.get(position).getUserList().get(2).getNickname();
            String profile3 = chatRoomItemArrayList.get(position).getUserList().get(2).getProfile();
            //채팅방 상대의 프로필 사진 설정
            ...
            viewBinderHelper.bind(((ChatListThreeViewHolder) holder).swipeLayout, String.valueOf(chatRoomItemArrayList.get(position).getRoomNum()));
            //스와이프하면 나오는 나가기 버튼 클릭 리스너
        ...
        //네명 이상과 대화하는 채팅방 리스트
            ...
            String nickname4 = chatRoomItemArrayList.get(position).getUserList().get(3).getNickname();
            String profile4 = chatRoomItemArrayList.get(position).getUserList().get(3).getProfile();
            //채팅방 상대의 프로필 사진 설정
            ...
            //채팅방 참여자 수 설정
            ((ChatListFourViewHolder) holder).tv_count.setText(String.valueOf(chatRoomItemArrayList.get(position).getUserList().size() + 1));
            ...
            viewBinderHelper.bind(((ChatListFourViewHolder) holder).swipeLayout, String.valueOf(chatRoomItemArrayList.get(position).getRoomNum()));
            //스와이프하면 나오는 나가기 버튼 클릭 리스너
        ...
        //채팅방에 자기만 남은 경우
            ...
            viewBinderHelper.bind(((ChatListNoneViewHolder) holder).swipeLayout, String.valueOf(chatRoomItemArrayList.get(position).getRoomNum()));
            //스와이프하면 나오는 나가기 버튼 클릭 리스너
    ...
    //홀더의 아이템 데이터에 변화가 생겼을 때 아이템 홀더 전체를 새로고침하는 게 아니라 변화가 생긴 부분만 바인딩 콜백 메소드
    @Override
