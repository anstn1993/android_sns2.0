...
public class UserAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public interface UserRecyclerViewListener {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof UserViewHolder){
            //유저의 프로필 사진 설정
            Glide.with(context).load("http://13.124.105.47/profileimage/" + userItemArrayList.get(position).getProfile())
                    .thumbnail(0.1f)
            ...
            //유저의 닉네임 설정
            ((UserViewHolder) holder).tv_nickname.setText(userItemArrayList.get(position).getNickname());
            //유저의 이름 설정
            ((UserViewHolder) holder).tv_name.setText(userItemArrayList.get(position).getName());
            //유저가 채팅방에 참여중인 사용자가 아닌 경우
            if(!userItemArrayList.get(position).getIsCurrentParticipant()) {
                //체크박스 설정
