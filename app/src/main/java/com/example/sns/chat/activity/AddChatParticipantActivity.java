...
import com.example.sns.chat.adapter.SelectedUserAdapter;
import com.example.sns.chat.model.SelectedUserItem;
import com.example.sns.chat.adapter.UserAdapter;
import com.example.sns.chat.model.UserItem;
...
public class AddChatParticipantActivity extends AppCompatActivity implements SelectedUserAdapter.SelectedUserRecyclerViewListener, UserAdapter.UserRecyclerViewListener, HttpRequest.OnHttpResponseListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            currentParticipantArrayList = new ArrayList<>();
            String participantString = getIntent().getStringExtra("participantString");
            if (!participantString.equals("null")) {
        ...
        tv_apply.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (selectedUserItemArrayList.size() != 0) {
                    try {
                        //새롭게 만들고자 하는 채팅방의 참여자 리스트를 json스트링으로 만들어서 넘겨준다(계정, 닉네임, 프로필)
                        ...
                        for (int i = 0; i < selectedUserItemArrayList.size(); i++) {
                            ...
                            participantData.put("account", account);
                        ...
                        Log.d("추가된 사용자 데이터", addedParticipantData.toString());
                        intent.putExtra("participantList", addedParticipantData.toString());
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    requestBody.put("searchText", searchText);
                    requestBody.put("lastId", userItemArrayList.get(userItemArrayList.size() - 2).getId());
                    if (searchText == null) {
                        requestBody.put("isSearched", false);
                    } else {
                        requestBody.put("isSearched", true);
                    }
                    HttpRequest httpRequest = new HttpRequest("GET", requestBody.toString(), "getchatuser.php", AddChatParticipantActivity.this::onHttpResponse);
                    httpRequest.execute();
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            if (selectedUserItemArrayList.size() != 0) {
                                for (int j = 0; j < selectedUserItemArrayList.size(); j++) {
                                    if (selectedUserItemArrayList.get(j).getNickname().equals(nickname)) {
                                        isSelected = true;
                                        break;
                                    }
                                }
                            ...
                            //이미 채팅에 참여하고 있는 사용자의 데이터인 경우에는 true 아니면 false로 설정
                            boolean isCurrentParticipant = false;
                            for (int j = 0; j < currentParticipantArrayList.size(); j++) {
                                if (currentParticipantArrayList.get(j).equals(account)) {
    ...
    public void onRemoveClicked(int position) {
        ...
        String nickname = selectedUserItemArrayList.get(position).getNickname();
        //체크박스 해제를 해야할 리스트의 index를 구하기 위해 반복문으로 index를 찾는다.
        int index = 0;
        for (int i = 0; i < userItemArrayList.size(); i++) {
            if (userItemArrayList.get(i).getNickname().equals(nickname)) {
                index = i;
                break;
            }
        }
        ...
        userItemArrayList.get(index).setIsSelected(false);
    ...
    public void onContainerClicked(int position) {
        if (userItemArrayList.get(position).getIsSelected()) {
            userItemArrayList.get(position).setIsSelected(false);
            userAdapter.notifyItemChanged(position, "false");
            String nickname = userItemArrayList.get(position).getNickname();
            //인덱스를 찾기 위한 반복문
            int index = 0;
            for (int i = 0; i < selectedUserItemArrayList.size(); i++) {
                if (nickname.equals(selectedUserItemArrayList.get(i).getNickname())) {
                    index = i;
                    break;
                }
            }
            ...
            userItemArrayList.get(position).setIsSelected(true);
            userAdapter.notifyItemChanged(position, "true");
            String profile = userItemArrayList.get(position).getProfile();
            String account = userItemArrayList.get(position).getAccount();
            String nickname = userItemArrayList.get(position).getNickname();
