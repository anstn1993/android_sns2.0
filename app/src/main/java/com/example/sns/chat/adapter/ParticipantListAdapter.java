...
public class ParticipantListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public interface ParticipantListRecyclerViewListener {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof AddUserItemViewHolder) {
            ...
            //참여자 프로필 사진 셋
            Glide.with(context).load("http://13.124.105.47/profileimage/" + chatParticipantItemArrayList.get(position-1).getProfile())
                    .thumbnail(0.1f)
            ...
            //참여자 닉네임 셋
            ((ChatParticipantItemViewHolder)holder).tv_nickname.setText(chatParticipantItemArrayList.get(position-1).getNickname());
        }
    }
