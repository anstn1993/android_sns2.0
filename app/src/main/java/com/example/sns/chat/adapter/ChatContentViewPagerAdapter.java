...
import com.example.sns.chat.fragment.ChatContentDetailFragment;
...
public class ChatContentViewPagerAdapter extends FragmentPagerAdapter {
    ...
    public Fragment getItem(int i) {
        ...
        String type = chatContentDetailItemArrayList.get(i).getType();//컨텐츠 타입(image or video)
        String contentUri = null;//컨텐츠 경로
        if (type.equals("image")) {//컨텐츠가 이미지인 경우
            contentUri = "http://13.124.105.47/chatimage/" + chatContentDetailItemArrayList.get(i).getContent();
        } else {//컨텐츠가 동영상인 경우
            contentUri = "http://13.124.105.47/chatvideo/" + chatContentDetailItemArrayList.get(i).getContent();
        }
