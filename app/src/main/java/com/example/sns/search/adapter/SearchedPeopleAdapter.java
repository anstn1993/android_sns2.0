...
import com.example.sns.search.model.SearchedPeopleItem;
...
public class SearchedPeopleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public interface SearchedPeopleRecyclerViewListener{
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof SearchedPeopleViewHolder){
            //프로필 사진 설정
            Glide.with(context).load("http://13.124.105.47/profileimage/"+searchedPeopleItemArrayList.get(position).getProfile())
                    .thumbnail(0.1f)
            ...
            //닉네임 설정
            ((SearchedPeopleViewHolder)holder).tv_nickname.setText(searchedPeopleItemArrayList.get(position).getNickname());
            //이름 설정
            ((SearchedPeopleViewHolder)holder).tv_name.setText(searchedPeopleItemArrayList.get(position).getName());
